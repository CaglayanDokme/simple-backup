...
- Pull requests targeting `master` and pushes to `master` trigger `.github/workflows/release-check.yml`, which requires the top changelog section to be a concrete versioned heading (e.g., `## v0.4.3 - April 26, 2026`). The check validates SemVer progression, release note extraction, and artifact build.
